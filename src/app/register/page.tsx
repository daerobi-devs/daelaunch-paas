    ...
    <div className="min-h-screen flex flex-col justify-center items-center px-6 bg-obsidian-950">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-mono font-bold text-white tracking-wide">Create Developer Account</h2>
          <p className="text-sm text-slate-400 mt-1 font-sans">Dapatkan akses deployment gratis dengan sandbox isolasi</p>
        </div>
        ...
        <div className="p-8 rounded-xl bg-obsidian-900/80 border border-obsidian-800 shadow-2xl backdrop-blur-xl">
          ...
          <div className="mb-6 p-3 rounded-lg bg-cyber-cyan/5 border border-cyber-cyan/20 flex items-start gap-2.5 text-xs text-slate-300 font-sans">
            <ShieldCheck className="w-4 h-4 text-cyber-cyan shrink-0 mt-0.5" />
            <span>
              <strong>Free Tier:</strong> Alokasi 2 aplikasi aktif, 512MB RAM limit per container, subdomain otomatis <code className="text-cyber-cyan font-mono">*.daeroom.my.id</code>.
            </span>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-mono text-slate-400 mb-2">USERNAME</label>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full font-mono text-sm font-bold bg-cyber-cyan text-black py-3 rounded-lg hover:bg-cyber-cyan/90 transition-all shadow-[0_0_20px_rgba(0,240,255,0.3)] disabled:opacity-50 flex items-center justify-center gap-2 mt-4"
            >
              {loading ? 'Creating Account...' : 'Initialize Free Tier'}
          ...
          <div className="mt-6 pt-6 border-t border-obsidian-800 text-center text-xs font-mono text-slate-400">
            Sudah punya akun?{' '}
            <Link href="/login" className="text-cyber-cyan hover:underline font-bold">
              Sign In ke Console
            </Link>
          </div>
        </div>
